...
function isDirectLink(url) {
    // Direct download patterns (Anchored to the start of the URL)
...
function isRedirectLink(url) {
    const redirectPatterns = [
        /dl\.php\?link=/i,                                // dl.php redirects
        /https?:\/\/[a-z0-9-]+\.hubcdn\.fans\/\?id=/i,    // Catches pixel.hubcdn, gpdl.hubcdn, etc.
        /https?:\/\/[a-z0-9-]+\.rohitkiskk\.workers\.dev/i, // Catches any rohitkiskk workers
        /\/go\//i,                                        // Generic /go/ redirects
        ...
        if (currentUrl.includes('dl.php?link=')) {
            try {
                ...
                if (targetLink && targetLink.startsWith('http')) {
                    ...
                    hopCount++;
                    continue; // Re-evaluate the extracted URL in the next loop
                }
            } catch (err) {
