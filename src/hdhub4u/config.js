...
// =================================================================================
// 🆕 PROXY TRANSFORMATION FUNCTIONS
// =================================================================================

/**
 * Check if a URL should be proxied through Cloudflare Worker
...
/**
 * Transform a URL to use Cloudflare Worker proxy
 * Direct URL passing - NO ENCODING
 */
export function transformToProxyUrl(url) {
    if (!shouldProxyUrl(url)) {
        return url; // Return original URL if proxy not needed
    }
    
    try {
        // Pass URL directly without encoding
 ...
 */
export function transformStreamUrls(streams) {
    if (!ENABLE_GOOGLE_DRIVE_PROXY || !Array.isArray(streams)) {
        return streams;
    }
    ...
    return streams.map(stream => {
        if (stream.url) {
            return {
                ...stream,
                url: transformToProxyUrl(stream.url)
            };
        }
        return stream;
    });
