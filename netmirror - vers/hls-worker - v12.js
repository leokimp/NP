...
export default {
  async fetch(request, _env) {
    ...
    // Forward Range header from the player to CDN
    const rangeHeader = request.headers.get('range');
    const isCDNDomain = new URL(upstreamUrl).hostname.includes('freecdn') || new URL(upstreamUrl).hostname.includes('nm-cdn');
    if (rangeHeader && isCDNDomain) {
      upstreamHeaders['Range'] = rangeHeader;
    }
    const customCookie = request.headers.get('x-nm-cookie');
    if (customCookie) {
