/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║              NetMirror — Nuvio Mobile Plugin  v12.1 (App API)                ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 ...
 * ║  Audio fix    › CDN master M3U8 already has #EXT-X-MEDIA AUDIO groups;       ║
 * ║                 worker rewrites all URLs so mpv gets a complete manifest     ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  v12.1 change › Cloudflare Worker proxy for external player support          ║
 * ║                 (mpvEx / any player that ignores injected headers)           ║
 * ║                 Worker fetches with headers → rewrites M3U8 URLs →           ║
 * ║                 player never needs to send Origin/Referer/X-Requested-With   ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
...
// ─── Proxy (v12.1) ───────────────────────────────────────────────────────────
//
// Set ENABLE_PROXY = true and point PROXY_WORKER_URL at your deployed worker.
//
// When true:   stream URL  = https://<worker>/proxy?url=<original>
//              headers     = {} (worker injects them server-side)
//              Works with: mpvEx, VLC, any external player — no header support needed.
//
// When false:  stream URL  = original net52.cc/mobile/hls/... URL
//              headers     = CDN headers (Origin, Referer, X-Requested-With)
//              Works with: Nuvio internal player only.
//
// You can keep ENABLE_PROXY = true always — Nuvio's internal player is also
// happy playing through the proxy since it's just a plain HTTPS URL.
//
...
// CDN headers — kept for reference / direct-play fallback (ENABLE_PROXY = false)
var CDN_HEADERS = {
  'User-Agent'         : APP_UA,
  'Origin'             : NM_BASE,
  'Referer'            : NM_BASE + '/',
...
/**
 * v12.1 — Build the URL that the player will actually receive.
 *
 * ENABLE_PROXY = true:
 *   Returns https://<worker>/proxy?url=<encoded original URL>
 *   The worker injects all required headers and rewrites the M3U8 recursively.
 *   Player needs zero custom headers → works with mpvEx / any external player.
 *
 * ENABLE_PROXY = false:
 *   Returns the original URL unchanged (legacy behaviour).
 *   Only works if the player can be passed headers (Nuvio internal player).
 */
function buildPlayerUrl(originalUrl) {
  if (!ENABLE_PROXY) return originalUrl;
  var base = PROXY_WORKER_URL.replace(/\/$/, '');
  return base + '/proxy?url=' + encodeURIComponent(originalUrl);
}

// ═════════════════════════════════════════════════════════════════════════════
// CACHE (worker — identical contract to v11/v12)
// ═════════════════════════════════════════════════════════════════════════════

function nmCacheKey(tmdbId, type, season, episode) {
  return 'nm121_' + tmdbId + '_' + (type || 'movie') +
...
function nmSetCachedStreams(tmdbId, type, season, episode, streams) {
  ...
  return fetch(CACHE_WORKER_URL, {
    ...
    body: JSON.stringify({
      ...
      metadata: { tmdbId: tmdbId, type: type, season: season, episode: episode, savedAt: Date.now(), plugin: 'netmirror-v12.1' },
    }),
  }).then(function (res) {
...
// ═════════════════════════════════════════════════════════════════════════════
// AUTH BYPASS — same as v11/v12. Per data.md §2.
// ═════════════════════════════════════════════════════════════════════════════
...
// ═════════════════════════════════════════════════════════════════════════════
// SEARCH ENGINE (unchanged from v12)
// ═════════════════════════════════════════════════════════════════════════════
    ...
    return request(url, { headers: hdrs })
      ...
      .then(function (data) {
        return (data.searchResult || []).map(function (r) {
          ...
          return score > 0 ? { id: r.id, title: r.t, score: score } : null;
        }).filter(Boolean);
      })
      .catch(function () { return []; });
  })).then(function (arrays) {
    var all = [].concat.apply([], arrays);
    var best = all.reduce(function (a, b) { return b.score > a.score ? b : a; }, { score: 0 });
 ...
 * v12.1 — makeStream
 *
 * Key change: when ENABLE_PROXY is true, the stream URL is wrapped through
 * the Cloudflare Worker proxy. The worker injects all CDN headers itself,
 * so `headers` in the returned object can be empty — the player never needs
 * to send Origin/Referer/X-Requested-With.
 *
 * This is the ONLY change needed in the plugin to support mpvEx and any
 * other external player that cannot forward custom headers.
 *
 * When ENABLE_PROXY is false, falls back to original behaviour: raw URL +
 * CDN headers for Nuvio's internal player.
 */
function makeStream(platform, qualityName, finalUrl, titleLine) {
  var playerUrl = buildPlayerUrl(finalUrl);

  // When proxying: no headers needed (worker handles injection).
  // When direct:   CDN headers required (Nuvio internal player only).
  ...
  console.log(PLUGIN_TAG + ' [stream] ' +
    (ENABLE_PROXY ? '[PROXIED] ' : '[DIRECT] ') +
    qualityName + ' → ' + playerUrl.slice(0, 80) + '…');
